...
(function initBgCanvas() {
  ...
  // Large soft nebula glows — purple / magenta / blue, smooth drifting
  let blobs = [
    { x: 0.18, y: 0.22, r: 560, color: 'rgba(139, 92, 246, 0.32)', sx: 0.00014, sy: 0.00010, phase: 0 },
    { x: 0.80, y: 0.32, r: 620, color: 'rgba(236, 72, 153, 0.28)', sx: 0.00010, sy: 0.00015, phase: 1.6 },
    { x: 0.30, y: 0.72, r: 580, color: 'rgba(56, 189, 248, 0.22)', sx: 0.00012, sy: 0.00013, phase: 3.1 },
    { x: 0.85, y: 0.85, r: 640, color: 'rgba(167, 64, 237, 0.26)', sx: 0.00015, sy: 0.00011, phase: 4.4 },
    { x: 0.5, y: 0.5, r: 720, color: 'rgba(99, 102, 241, 0.18)', sx: 0.00009, sy: 0.00016, phase: 2.2 },
    { x: 0.08, y: 0.85, r: 500, color: 'rgba(94, 196, 182, 0.16)', sx: 0.00013, sy: 0.00009, phase: 5.0 }
  ];
  if (isMobile) blobs = blobs.slice(0, 4);

  // Glowing particle field — pink / magenta / violet / blue dots that drift + twinkle
  const PARTICLE_COUNT = isMobile ? 70 : 160;
  const particles = [];
  const colors = [
    '236, 72, 153',   // pink/magenta
    '167, 64, 237',   // violet
    '99, 102, 241',   // blue-violet
    '94, 196, 182',   // teal accent
    '244, 100, 130',  // rose
    '139, 92, 246'    // purple
  ];
  ...
  function draw() {
    ...
    // nebula glows
    blobs.forEach(b => {
      const x = (b.x + Math.sin(frame * b.sx + b.phase) * 0.14) * w;
      const y = (b.y + Math.cos(frame * b.sy + b.phase) * 0.14) * h;
    ...
    // glowing particles with twinkle
    particles.forEach(p => {
      p.x += p.vx;
      p.y += p.vy;
      ...
      const twinkle = (Math.sin(frame * p.twSpeed + p.twPhase) + 1) / 2;
      const o = p.baseO * (0.5 + twinkle * 0.5);
      const x = p.x * w;
      const y = p.y * h;

      const glow = ctx.createRadialGradient(x, y, 0, x, y, p.r * 6);
      glow.addColorStop(0, `rgba(${p.color}, ${o})`);
      ...
      ctx.fillStyle = `rgba(${p.color}, ${Math.min(o + 0.35, 1)})`;
      ctx.beginPath();
...
if (cursorDot && cursorRing && window.matchMedia('(hover: hover) and (pointer: fine)').matches) {
  ...
  window.addEventListener('mousemove', (e) => {
    ...
    if (!cursorReady) {
      ...
      cursorRing.classList.add('ready');
    }
    // Dot follows instantly
    cursorDot.style.transform = `translate3d(${tx}px, ${ty}px, 0) translate(-50%, -50%)`;
  }, { passive: true });
  ...
  // Link/button variant: distinct accent ring
...
/* ============================================
   ...
   ============================================ */
const fadeEls = document.querySelectorAll('.fade-in, .fade-left, .fade-right, .fade-up-stagger');
...
(async function loadGithub() {
  ...
  try {
    const res = await fetch('https://api.github.com/users/Aryan-developer613', { cache: 'no-store' });
    if (!res.ok) throw new Error('GitHub API error: ' + res.status);
    const data = await res.json();

    if (typeof data.public_repos !== 'number') throw new Error('Unexpected GitHub response');

    const values = [data.public_repos, data.followers ?? 0, data.following ?? 0];
    stats.forEach((el, i) => {
    ...
    try {
      const reposRes = await fetch('https://api.github.com/users/Aryan-developer613/repos?sort=updated&per_page=100', { cache: 'no-store' });
      if (reposRes.ok) {
...
// --- LeetCode ---
/* ============================================
   CODOLIO AGGREGATE (from LeetCode + Codeforces)
   ============================================ */
const codolioAggregate = { leetcodeTotal: null, cfRating: null };

function updateCodolioCard() {
  const totalEl = document.querySelector('#codolioStats .cstat-num');
  const cfRatingEl = document.getElementById('codolioCfRating');
  if (!totalEl || !cfRatingEl) return;

  if (codolioAggregate.leetcodeTotal !== null) {
    const target = codolioAggregate.leetcodeTotal;
    const obs = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
  ...
  if (codolioAggregate.cfRating !== null) {
    cfRatingEl.textContent = codolioAggregate.cfRating || 'Unrated';
  } else {
...
(async function loadLeetcode() {
  ...
  codolioAggregate.leetcodeTotal = total;
  updateCodolioCard();
})();
...
(async function loadCodeforces() {
  ...
  try {
    ...
    codolioAggregate.cfRating = rating || 0;
    updateCodolioCard();

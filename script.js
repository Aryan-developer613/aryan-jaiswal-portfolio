...
if (cursorDot) {
  ...
  document.querySelectorAll('a, button, .pill, .project-card, .code-card, .stat-card').forEach(el => {
...
(async function loadLeetcode() {
  ...
  if (badgeWrap) {
    ...
    if (data.ranking) {
      ...
      rb.className = 'badge';
      rb.textContent = `Global Rank #${data.ranking.toLocaleString()}`;
      badgeWrap.appendChild(rb);
    }
  }
})();

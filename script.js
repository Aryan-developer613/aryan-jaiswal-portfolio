...
(function initBgCanvas() {
  ...
  // Large soft nebula glows — purple / magenta / blue, slow drifting
  const blobs = [
    { x: 0.18, y: 0.22, r: 520, color: 'rgba(124, 58, 237, 0.16)', sx: 0.00010, sy: 0.00008, phase: 0 },
    { x: 0.78, y: 0.35, r: 600, color: 'rgba(236, 72, 153, 0.13)', sx: 0.00008, sy: 0.00011, phase: 1.6 },
    { x: 0.32, y: 0.7, r: 560, color: 'rgba(94, 196, 182, 0.08)', sx: 0.00009, sy: 0.00010, phase: 3.1 },
    { x: 0.85, y: 0.82, r: 620, color: 'rgba(124, 58, 237, 0.14)', sx: 0.00011, sy: 0.00009, phase: 4.4 },
    { x: 0.5, y: 0.5, r: 700, color: 'rgba(94, 110, 240, 0.10)', sx: 0.00007, sy: 0.00012, phase: 2.2 }
  ];
  ...
  function draw() {
    ...
    // nebula glows
    blobs.forEach(b => {
      const x = (b.x + Math.sin(frame * b.sx + b.phase) * 0.08) * w;
      const y = (b.y + Math.cos(frame * b.sy + b.phase) * 0.08) * h;
    ...
    particles.forEach(p => {
      ...
      ctx.fillStyle = `rgba(${p.color}, ${Math.min(o + 0.3, 1)})`;
      ctx.beginPath();
